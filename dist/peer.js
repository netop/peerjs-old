...
Negotiator._setupListeners = function(connection, pc, pc_id) {
  ...
  pc.oniceconnectionstatechange = function() {
    switch (pc.iceConnectionState) {
      case 'failed':
        ...
        break;
      case 'connected':
        util.log('iceConnectionState is connected' + peerId);
        clearTimeout(connection.disconnectedTimeout);
        break;
      case 'disconnected':
        util.log('iceConnectionState is disconnected, closing connections to ' + peerId);
        connection.reconnectable && (connection.disconnectedTimeout || (connection.disconnectedTimeout = setTimeout(function(){ connection.open && connection.close();}, 15000)));
        !connection.reconnectable && connection.close();
        break;
      case 'closed':
        util.log('iceConnectionState is closed, closing connections to ' + peerId);
  ...
  // Workaround for Chrome: skip nested negotiations for multi-track streams
  pc.onsignalingstatechange = function() {
    connection.isNegotiating = (pc.signalingState != "stable");
  }
  ...
  pc.onnegotiationneeded = function() {
    ...
    util.log('`negotiationneeded` triggered. is negotiating:' + !!connection.isNegotiating);

    // Workaround for Chrome: skip nested negotiations for multi-track streams
    if(connection.isNegotiating) {return;}
...
Negotiator._makeOffer = function(connection) {
  ...
  pc.createOffer(function(offer) {
    ...
    if (connection.type === 'data') {
      offer.sdp = Negotiator.higherBandwidthSDP(offer.sdp);
    }
...
Negotiator.higherBandwidthSDP = function(sdp) {
...
Negotiator._makeAnswer = function(connection) {
  ...
  pc.createAnswer(function(answer) {
    ...
    // if (!util.supports.sctp && connection.type === 'data' && connection.reliable) {
    if (connection.type === 'data') {
      answer.sdp = Negotiator.higherBandwidthSDP(answer.sdp);
    }
